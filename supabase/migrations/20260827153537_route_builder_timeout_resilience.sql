-- Route planning must stay responsive even as inventory and VMS history grow.
-- Keep a narrow storage balance view for planning/validation and make the latest
-- VMS view read only the single active stock batch instead of every historical row.

create index if not exists idx_inventory_movements_created_at_desc
  on public.inventory_movements (created_at desc);

create or replace view public.route_storage_stock_by_product
with (security_invoker = true)
as
with movement_deltas as (
  select product_id, -quantity as quantity_delta
  from public.inventory_movements
  where from_entity_type = 'storage'

  union all

  select product_id, quantity as quantity_delta
  from public.inventory_movements
  where to_entity_type = 'storage'
)
select
  product_id,
...
revoke all on table public.route_storage_stock_by_product from public, anon;
grant select on table public.route_storage_stock_by_product to authenticated, service_role;
...
alter view public.latest_vms_stock_by_slot set (security_invoker = true);
revoke all on table public.latest_vms_stock_by_slot from public, anon;
grant select on table public.latest_vms_stock_by_slot to authenticated, service_role;

create or replace view public.refill_recommendations as
with storage_stock as (
  select product_id, quantity_on_hand as available_storage_qty
  from public.route_storage_stock_by_product
),
vms_stock as (
  select
...
matched_slots as (
  ...
  left join public.machine_slots ms
   ...
   and ms.active = true
   and ((v.slot_code is not null and ms.slot_code = v.slot_code) or v.slot_code is null)
  order by
    v.id,
...
alter view public.refill_recommendations set (security_invoker = true);
revoke all on table public.refill_recommendations from public, anon;
grant select on table public.refill_recommendations to authenticated, service_role;

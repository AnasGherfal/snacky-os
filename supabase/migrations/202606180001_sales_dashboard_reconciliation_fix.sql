...
with detailed_sales as (
  select
    ...
    ((coalesce(tx.payment_time, tx.delivery_time) at time zone 'Africa/Tripoli'))::date as sale_date,
    date_trunc('month', (coalesce(tx.payment_time, tx.delivery_time) at time zone 'Africa/Tripoli'))::date as sales_month,
    coalesce(vib.report_start_date, ((coalesce(tx.payment_time, tx.delivery_time) at time zone 'Africa/Tripoli'))::date) as report_start_date,
    coalesce(vib.report_end_date, ((coalesce(tx.payment_time, tx.delivery_time) at time zone 'Africa/Tripoli'))::date) as report_end_date,
...
as $$
  ...
  normalized_rows as (
    select
      ...
      case
        when coalesce(tx.payment_time, tx.delivery_time) is null then null
        else ((coalesce(tx.payment_time, tx.delivery_time) at time zone 'Africa/Tripoli'))::date
      end as sale_business_date,
    ...
    select
      ...
      count(*) filter (
        where nr.transaction_status = 'successful_sale'
          and nr.transaction_at is not null
          and (p_date_from is null or nr.sale_business_date >= p_date_from)
          and (p_date_to is null or nr.sale_business_date <= p_date_to)

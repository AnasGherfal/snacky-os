-- Introduce a single-signature pickup RPC with no overloaded name collisions.
-- This version preserves the soft-retire route-pick-list behavior and keeps the
-- pickup confirmation atomic, idempotent, and auditable.

create or replace function public.snacky_confirm_route_pickup_batch_v2(
  p_route_id uuid,
...
declare
  ...
  v_existing_checked_pickup_line_ids uuid[] := '{}'::uuid[];
  v_new_checked_pickup_line_ids uuid[] := '{}'::uuid[];
  v_authoritative_checked_pickup_line_ids uuid[] := '{}'::uuid[];
...
begin
  ...
  if p_expected_route_status is not null and v_route.status <> p_expected_route_status then
    raise exception 'Route status changed from % to %. Refresh the route before confirming pickup.',
      p_expected_route_status::text,
      v_route.status::text using errcode = 'P0001';
  end if;
  ...
  from jsonb_to_recordset(v_pick_list_rows)
    as x(route_stop_item_id uuid, is_checked boolean, planned_qty integer)
  ...
  into v_new_checked_pickup_line_ids
  from unnest(v_checked_pickup_line_ids) as checked_id
  join jsonb_to_recordset(v_new_stop_item_rows) as x(id uuid, route_stop_id uuid)
    on x.id = checked_id
  where x.id is not null
    and (coalesce(array_length(p_selected_stop_ids, 1), 0) = 0 or x.route_stop_id = any(p_selected_stop_ids));

  select coalesce(array_agg(distinct rsi.id order by rsi.id), '{}'::uuid[])
  into v_existing_checked_pickup_line_ids
  from unnest(v_checked_pickup_line_ids) as checked_id
  ...
  into v_authoritative_checked_pickup_line_ids
  from unnest(v_existing_checked_pickup_line_ids || v_new_checked_pickup_line_ids) as x;

  select coalesce(
    array_agg(distinct acknowledged_id order by acknowledged_id),
    '{}'::uuid[]
  )
  into v_acknowledged_pickup_line_ids
  from unnest(coalesce(p_acknowledged_pickup_line_ids, '{}'::uuid[])) as acknowledged_id
  where acknowledged_id is not null;

  if coalesce(array_length(v_acknowledged_pickup_line_ids, 1), 0) = 0
     and coalesce(array_length(v_authoritative_checked_pickup_line_ids, 1), 0) > 0 then
    v_acknowledged_pickup_line_ids := v_authoritative_checked_pickup_line_ids;
  end if;
  ...
  where not (required_id = any(v_authoritative_checked_pickup_line_ids));
  ...
  where not (acknowledged_id = any(v_authoritative_checked_pickup_line_ids));

  if v_invalid_count > 0 then
    raise exception 'Pickup checklist contains a stale or foreign route line.' using errcode = 'P0001';
  end if;

  select count(*)
  into v_invalid_count
  from unnest(v_authoritative_checked_pickup_line_ids) as checked_id
  ...
  if jsonb_array_length(v_new_stop_item_rows) > 0 then
    ...
    on conflict (id) do update set
      ...
      machine_slot_id = excluded.machine_slot_id,
...
grant execute on function public.snacky_confirm_route_pickup_batch_v2(
  uuid,
  public.route_status,
  public.route_status,
  timestamptz,

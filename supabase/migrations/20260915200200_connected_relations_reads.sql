...
-- An activity inherits its most specific parent, never broader location access.
-- A colleague may see a location but not another employee's rent obligation.
create or replace function public.snacky_crm_activity_visible(p_task uuid,p_obligation uuid,p_issue uuid,p_lead uuid,p_contact uuid,p_location uuid)
returns boolean language sql stable security definer set search_path=public,pg_catalog as $$
 select case
  when p_task is not null then public.snacky_crm_allowed('task',p_task)
  when p_obligation is not null then public.snacky_crm_allowed('obligation',p_obligation)
  when p_issue is not null then public.snacky_crm_allowed('issue',p_issue)
  when p_lead is not null then public.snacky_crm_allowed('lead',p_lead)
  when p_contact is not null then public.snacky_crm_allowed('contact',p_contact)
  else public.snacky_crm_allowed('location',p_location) end;
$$;
revoke all on function public.snacky_crm_activity_visible(uuid,uuid,uuid,uuid,uuid,uuid) from public,anon;
grant execute on function public.snacky_crm_activity_visible(uuid,uuid,uuid,uuid,uuid,uuid) to authenticated;

create or replace function public.snacky_crm_timeline_v1(p_kind text,p_id uuid,p_offset integer default 0)
returns jsonb language plpgsql stable security definer set search_path=public,pg_catalog as $$
declare source_leads uuid[];result jsonb;
begin
 if not public.snacky_crm_allowed(p_kind,p_id) then raise exception 'Record access denied' using errcode='42501';end if;
 source_leads:=case when p_kind='location' then array(select id from public.location_pipeline_leads where converted_location_id=p_id and public.snacky_crm_allowed('lead',id)) else array[]::uuid[] end;
 with events as (
  select a.id,a.activity_type,a.summary,a.actor_id,a.actor_name,a.occurred_at,a.before_data,a.after_data
  from public.crm_activities a
  where (case p_kind when 'lead' then a.lead_id=p_id when 'issue' then a.issue_id=p_id when 'location' then a.location_id=p_id or a.lead_id=any(source_leads) when 'contact' then a.contact_id=p_id when 'obligation' then a.obligation_id=p_id when 'task' then a.task_id=p_id else false end)
  and public.snacky_crm_activity_visible(a.task_id,a.obligation_id,a.issue_id,a.lead_id,a.contact_id,a.location_id)
  union all
  select a.id,a.activity_type,a.summary,a.created_by_user_id,t.full_name,a.occurred_at,null::jsonb,null::jsonb
  from public.location_pipeline_activities a left join public.team_members t on t.id=a.created_by_user_id
  where p_kind in ('lead','location') and (a.lead_id=p_id or a.lead_id=any(source_leads))
  and public.snacky_crm_allowed('lead',a.lead_id)
  and not exists(select 1 from public.crm_activities x where x.lead_id=a.lead_id and x.summary=a.summary and abs(extract(epoch from(x.created_at-a.created_at)))<5)
 ) select jsonb_build_object('total',(select count(*) from events),'offset',greatest(0,coalesce(p_offset,0)),'page_size',40,
  'rows',coalesce((select jsonb_agg(x order by x.occurred_at desc,x.id desc) from (select * from events order by occurred_at desc,id desc limit 40 offset greatest(0,coalesce(p_offset,0))) x),'[]'::jsonb)) into result;
 return result;
end $$;
revoke all on function public.snacky_crm_timeline_v1(text,uuid,integer) from public,anon;
grant execute on function public.snacky_crm_timeline_v1(text,uuid,integer) to authenticated;
...
begin
 ...
 with permitted as (
  ...
  select * from permitted r where
   ...
   and (nullif(p_filters->>'created_to','') is null or r.created_at<((p_filters->>'created_to')::date+1))
...
create policy crm_documents_read on storage.objects for select to authenticated using(
 ...
 and case when split_part(name,'/',3)~'^[0-9a-fA-F-]{36}$' then public.snacky_crm_allowed(split_part(name,'/',2),split_part(name,'/',3)::uuid) else false end
 and (split_part(name,'/',1)=auth.uid()::text or exists(select 1 from public.crm_documents d where d.object_path=name))
);

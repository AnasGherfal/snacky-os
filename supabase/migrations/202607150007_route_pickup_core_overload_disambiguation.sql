-- Resolve SQLSTATE 42725 caused by overloaded pickup core functions with defaults.
--
-- Production can contain both 15- and 16-argument
-- public.confirm_route_pickup_batch_core overloads. A positional 15-argument
-- call is ambiguous because the 16-argument overload also has defaults.
--
-- Keep the public function signature and defaults unchanged, derive the
-- acknowledgement set from the authoritative checked rows, and call the
-- 16-argument core with named notation. The named acknowledgement parameter
-- exists only on the 16-argument overload, so PostgreSQL has one candidate.
--
-- This migration changes function definitions only. It does not modify route,
-- pickup batch, checklist, inventory movement, or audit rows.
...
begin
  ...
  -- Every real positive-quantity route line must be checked. The redundant
  -- browser acknowledgement array is not a second route-blocking source.
  select count(*)
  ...
  -- Reject checked IDs from another route/stop, while allowing deterministic
  -- manual lines inserted in this same atomic confirmation.
  select count(*)
  into v_invalid_count
  ...
  -- Retained only for API compatibility/diagnostics.
  ...
  select coalesce(
    array_agg(
      distinct (coalesce(x.product_id, '') || ':' || coalesce(x.quantity, 0)::text)
  ...
  -- Named notation forces the unique 16-argument overload. In particular,
  -- p_acknowledged_pickup_line_ids does not exist on the 15-argument bridge.
  select *
  into v_result
  from public.confirm_route_pickup_batch_core(
    p_route_id => p_route_id,
    p_expected_route_status => p_expected_route_status,
    p_next_route_status => p_next_route_status,
    p_started_at => p_started_at,
    p_replace_pick_list => p_replace_pick_list,
    p_pickup_batch => p_pickup_batch,
    p_batch_stop_ids => p_batch_stop_ids,
    p_new_stop_item_rows => p_new_stop_item_rows,
    p_inventory_movements => p_inventory_movements,
    p_pick_list_rows => p_pick_list_rows,
    p_stock_line_rows => p_stock_line_rows,
    p_stop_item_picks => p_stop_item_picks,
    p_refill_line_picks => p_refill_line_picks,
    p_selected_stop_ids => p_selected_stop_ids,
    p_acknowledged_pickup_line_ids => v_checked_pickup_line_ids,
    p_selected_machine_ids => p_selected_machine_ids
  );

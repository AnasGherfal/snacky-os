-- Fix false pickup acknowledgement mismatches in the two-stage route pickup flow.
--
-- The previous wrapper compared the raw acknowledgement array length with every
-- checked pick-list row. That is not a valid comparison because:
--   1. zero-quantity route rows may still exist in the UI acknowledgement state
--      but are intentionally omitted from p_pick_list_rows; and
--   2. manually added stop products receive a deterministic route_stop_item_id
--      on the server, so the client cannot acknowledge that generated ID before
--      the final confirmation request.
--
-- This migration keeps strict server validation for every real existing route
-- pickup line while ignoring harmless extra acknowledgement IDs and permitting
-- server-generated manual line IDs. Required existing route lines must still be
-- both checked in p_pick_list_rows and explicitly acknowledged by the client.
...
declare
  ...
  v_checked_existing_pickup_line_ids uuid[] := '{}'::uuid[];
...
begin
  ...
  -- Normalize duplicate acknowledgement IDs before validation.
  select coalesce(array_agg(distinct acknowledged_id order by acknowledged_id), '{}'::uuid[])
  into v_acknowledged_pickup_line_ids
  from unnest(v_acknowledged_pickup_line_ids) as acknowledged_id
  where acknowledged_id is not null;

  -- Backwards compatibility for clients that only submit checked rows.
  ...
  -- Required lines are the existing positive planned route lines for the selected
  -- stops. Zero-quantity rows do not block pickup confirmation.
  ...
  -- Only compare explicit acknowledgements against checked IDs that already
  -- exist as route_stop_items. Server-generated manual line IDs in
  -- p_new_stop_item_rows are intentionally excluded because the browser cannot
  -- know those IDs before this transaction.
  select coalesce(array_agg(distinct rsi.id order by rsi.id), '{}'::uuid[])
  into v_checked_existing_pickup_line_ids
  from unnest(v_checked_pickup_line_ids) as checked_id
  join public.route_stop_items rsi on rsi.id = checked_id
  join public.route_stops rs on rs.id = rsi.route_stop_id
  where rs.route_id = p_route_id
    and (
      coalesce(array_length(p_selected_stop_ids, 1), 0) = 0
  ...
  -- Every required existing route line must be represented as checked in the
  -- authoritative pick-list payload.
  select count(*)
  ...
  -- Every required existing route line must also be explicitly acknowledged.
  select count(*)
  into v_invalid_count
  from unnest(v_required_pickup_line_ids) as required_id
  where not (required_id = any(v_acknowledged_pickup_line_ids));
  if v_invalid_count > 0 then
    raise exception 'Every required pickup line must be acknowledged before confirming pickup.' using errcode = 'P0001';
  end if;

  -- Any existing checked route line must be included in the explicit
  -- acknowledgement set. Harmless extra acknowledgement IDs are ignored.
  select count(*)
  into v_invalid_count
  from unnest(v_checked_existing_pickup_line_ids) as checked_id

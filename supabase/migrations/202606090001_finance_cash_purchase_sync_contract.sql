-- Standardize finance source links and make purchase/cash finance sync complete.
-- Standard going forward:
--   purchases: source_type='purchase', source_id=purchase_orders.id, linked_purchase_id=purchase_orders.id
--   cash collections: source_type='cash_collection', source_id=cash_collections.id, linked_cash_collection_id=cash_collections.id

alter table public.purchase_orders
  add column if not exists currency text default 'LYD';

-- Drop old uniqueness guards before normalizing legacy links; they are recreated after duplicates are voided.
drop index if exists public.financial_transactions_source_type_source_id_uidx;
drop index if exists public.financial_transactions_purchase_source_uidx;
drop index if exists public.financial_transactions_cash_collection_source_uidx;
drop index if exists public.financial_transactions_linked_purchase_id_uidx;
drop index if exists public.financial_transactions_linked_cash_collection_id_uidx;
alter table public.financial_transactions drop constraint if exists financial_transactions_linked_purchase_id_key;
alter table public.financial_transactions drop constraint if exists financial_transactions_linked_cash_collection_id_key;
...
-- Compatibility/backfill from old related_* names, then normalize source links.
update public.financial_transactions
set linked_purchase_id = related_purchase_id,
    source_type = 'purchase',
    source_id = related_purchase_id,
    updated_at = now()
where linked_purchase_id is null
  and related_purchase_id is not null;

update public.financial_transactions
set linked_cash_collection_id = related_cash_collection_id,
    source_type = 'cash_collection',
    source_id = related_cash_collection_id,
    updated_at = now()
where linked_cash_collection_id is null
  and related_cash_collection_id is not null;

update public.financial_transactions
set linked_purchase_id = source_id,
    updated_at = now()
where linked_purchase_id is null
...
update public.financial_transactions
set linked_cash_collection_id = source_id,
    updated_at = now()
where linked_cash_collection_id is null
  and source_type = 'cash_collection'
  and source_id is not null;

update public.financial_transactions
set source_type = 'purchase',
    source_id = linked_purchase_id,
    direction = 'money_out',
    ...
    transaction_status = case when coalesce(is_void, false) or voided_at is not null then 'voided' else coalesce(nullif(trim(transaction_status), ''), 'active') end,
...
set source_type = 'cash_collection',
    source_id = linked_cash_collection_id,
    direction = 'money_in',
    transaction_kind = 'cash_collection',
    transaction_type = coalesce(nullif(trim(transaction_type), ''), 'Revenue'),
    category = coalesce(nullif(trim(category), ''), 'Revenue'),
...
as $$
  select coalesce(p_purchase.status, '') not in ('cancelled', 'voided')
     and coalesce(p_purchase.payment_status, 'paid') in ('paid', 'confirmed')
$$;
...
-- Collapse duplicate active source links before enforcing uniqueness.
with duplicate_source_rows as (
  ...
  where source_type in ('purchase', 'cash_collection')
    and source_id is not null
)
update public.financial_transactions ft
set transaction_status = 'voided',
    is_void = true,
    voided_at = coalesce(ft.voided_at, now()),
    void_reason = coalesce(ft.void_reason, 'Duplicate finance source link removed by finance source-link standardization'),
    source_type = 'manual',
    source_id = null,
    linked_purchase_id = null,
    linked_cash_collection_id = null,
...
set transaction_status = 'voided',
    is_void = true,
    voided_at = coalesce(ft.voided_at, now()),
    void_reason = coalesce(ft.void_reason, 'Duplicate purchase finance link removed by finance source-link standardization'),
    source_type = 'manual',
    source_id = null,
    linked_purchase_id = null,
    updated_at = now()
...
set transaction_status = 'voided',
    is_void = true,
    voided_at = coalesce(ft.voided_at, now()),
    void_reason = coalesce(ft.void_reason, 'Duplicate cash collection finance link removed by finance source-link standardization'),
    source_type = 'manual',
    source_id = null,
    linked_cash_collection_id = null,
    updated_at = now()
...
create unique index if not exists financial_transactions_purchase_source_uidx
  on public.financial_transactions(source_type, source_id)
  where source_type = 'purchase' and source_id is not null;

create unique index if not exists financial_transactions_cash_collection_source_uidx
  on public.financial_transactions(source_type, source_id)
  where source_type = 'cash_collection' and source_id is not null;
...
drop policy if exists "financial_transactions_select_purchase_cash_sources" on public.financial_transactions;
...
create policy "financial_transactions_select_purchase_cash_sources"
  on public.financial_transactions
  for select
  to authenticated
  using (
    public.snacky_current_profile_has_any_role(array['owner', 'admin', 'finance'])
    and source_type in ('purchase', 'cash_collection')
  );
...
-- Real full backfill at migration time, not a date window or partial repair.

...
export default async function Page({params}:{params:Promise<{id:string}>}){
 ...
 const list=view.data.record;if(!list)notFound();const items=sourceAwareItems(list.items,view.sources);const totals=buyingTotals(items);const purchaseFlow=view.purchaseFlow;const purchaseFlowStatus=view.purchaseFlowStatus;
 return <section className={styles.workspace} dir={ar?'rtl':'ltr'} id="buying-list-detail">
  ...
  {list.status==='completed'&&purchaseFlow?<section className={styles.admin}>
   ...
   {!purchaseFlow.groups.length?<p>{ar?'لم يتم شراء أي منتجات من هذه القائمة، لذلك لا توجد عملية شراء لتسجيلها.':'No products were bought from this list, so there is no purchase to record.'}</p>:<div className={styles.itemCards}>{purchaseFlow.groups.map(group=><div className={styles.itemCard} key={group.supplier_id}>
    ...
    {group.linked_purchase?.purchase_id?<Link className={styles.secondary} href={`/purchases/${group.linked_purchase.purchase_id}`}>{ar?'فتح عملية الشراء المسجلة':'Open recorded purchase'} · {group.linked_purchase.status}</Link>:purchaseFlow.can_record?<Link className={styles.primary} href={`/purchases/new?source=${encodeURIComponent(buyingPurchaseSource(id,group.supplier_id))}`}>{ar?`سجّل واستلم من ${group.store_name}`:`Record & receive from ${group.store_name}`}</Link>:<p className={styles.hint}>{ar?'يجب أن يكون المشتري المسند لديه صلاحية المشتريات والاستلام.':'The assigned buyer needs purchase and receiving access.'}</p>}
   </div>)}</div>}
  </section>:list.status==='completed'&&purchaseFlowStatus==='not_installed'?<p className={styles.notice}>{ar?'ربط القائمة بالمشتريات غير مفعّل بعد. القائمة نفسها محفوظة ولم تتغير.':'Buying-to-storage linking is not installed yet. The checklist remains unchanged.'}</p>:null}

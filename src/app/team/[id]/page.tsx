...
export default async function TeamMemberProfilePage({
  ...
  if (!profile) {
    redirect(`/login?next=${encodeURIComponent(`/team/${id}`)}`);
  }
  ...
  const { data: member, error: memberError } = await client
    .from("team_members")
    .select(
      "id, full_name, email, phone, role, roles, active, active_status",
    )
    .eq("id", id)
    .maybeSingle();
  ...
  if (memberError || !member) {
    return (
      <ErrorState
        ...
        body="This team member could not be loaded."
        action={
          manager ? (
            <SecondaryButton href="/team">Back to team</SecondaryButton>
          ) : undefined
        }
      />
    );
  ...
  const { data: routes, error: routesError } = await client
    .from("routes")
    .select(
      "id, route_date, status, operator_id, created_at, started_at, completed_at",
    )
    .eq("operator_id", id)
  ...
  const [
    ...
    routeIds.length
      ? client
          ...
          .select(
            "id, route_id, machine_id, product_id, missing_product_name, actual_qty, action_type, created_at, product:products!route_stop_fill_lines_product_id_fkey(name)",
          )
          .in("route_id", routeIds)
          .order("created_at", { ascending: false })
          .limit(1000)
    ...
    routeIds.length
      ? client
          ...
          .select(
            "id, route_id, machine_id, product_name, quantity, total_amount_lyd, payment_method, sale_time, status",
          )
          .in("route_id", routeIds)
    ...
    routeIds.length
      ? client
          ...
          .select(
            "id, route_id, machine_id, adjustment_type, product_name, quantity, reason, notes, status, created_at",
          )
          .in("route_id", routeIds)
          .order("created_at", { ascending: false })
          .limit(1000)
    ...
    routeIds.length
      ? client
          ...
          .select(
            "id, related_route_id, related_machine_id, quantity, reason, movement_type, from_entity_type, to_entity_type, created_at, product:products(name)",
          )
  ...
  const machineIds = Array.from(
    new Set(stops.map((row: any) => row.machine_id).filter(Boolean)),
  );
  ...
  stops.forEach((stop: any) =>
    stopsByRoute.set(stop.route_id, [
      ...(stopsByRoute.get(stop.route_id) ?? []),
      stop,
    ]),
  );
  ...
  const sales = salesResult.error
    ? []
    : (salesResult.data ?? []).filter(isVisibleHistoryRow);
  const adjustments = adjustmentsResult.error
    ? []
    : (adjustmentsResult.data ?? []).filter(isVisibleHistoryRow);
  const movements = movementsResult.error ? [] : (movementsResult.data ?? []);
  const damaged = adjustments.filter(
    (row: any) => historyStatus(row.adjustment_type) === "damaged",
  );
  const returned = adjustments.filter(
    (row: any) =>
      historyStatus(row.adjustment_type) === "returned_from_machine",
  );
  const machineStorage = movements.filter((row: any) => {
    const reason = historyStatus(row.reason);
    return (
      row.to_entity_type === "machine_storage" ||
      row.movement_type === "route_to_machine_storage" ||
      reason === "extra_stock_left_at_machine" ||
      reason === "machine_storage"
    );
  });
  const completedRoutes = (routes ?? []).filter((route: any) =>
    ["completed", "verified", "payroll_pending", "paid", "reviewed"].includes(
      historyStatus(route.status),
    ),
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={`${normalizeRoles(member.roles, member.role).join(", ")} · ${member.email ?? "No email"}`}
        breadcrumbs={
          manager
            ? [
      ...
      {historyLoadError ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Some operator history could not load. Available profile details are still
          shown.
        </div>
      ) : null}
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        ...
        <SectionCard>
          <div className="p-4">
            <div className="text-sm text-slate-500">Machines visited</div>
            <div className="mt-1 text-2xl font-semibold">
              {machineIds.length}
            </div>
          </div>
        ...
        <SectionCard>
          <div className="p-4">
            <div className="text-sm text-slate-500">Units filled</div>
            <div className="mt-1 text-2xl font-semibold">
              {sum(fills, "actual_qty")}
            </div>
          </div>
        </SectionCard>
        <SectionCard>
          <div className="p-4">
            <div className="text-sm text-slate-500">Manual sales</div>
            <div className="mt-1 text-2xl font-semibold">
              {lyd(sum(sales, "total_amount_lyd"))}
            </div>
          </div>
        </SectionCard>
        <SectionCard>
          <div className="p-4">
            <div className="text-sm text-slate-500">Damaged / returned</div>
            <div className="mt-1 text-2xl font-semibold">
              {sum(damaged, "quantity")} / {sum(returned, "quantity")}
            </div>
          </div>
        </SectionCard>
      </div>
      ...
      <section className="surface-card p-4">
        ...
        {!(routes ?? []).length ? (
          <EmptyState
            title="No routes"
            body="No routes are assigned to this team member."
          />
        ) : (
          <DataTable
            headers={[
          ...
          >
            {(routes ?? []).map((route: any) => {
              ...
              const routeFills = fills.filter(
                (row: any) => row.route_id === route.id,
              );
              ...
              return (
                <tr key={route.id}>
                      ...
                      {routeStops
                        .map((stop: any) =>
                          formatMachineDisplayName(
                            machineById.get(stop.machine_id) ?? null,
                            { includeArea: true },
                          ),
                        )
                        .join(" · ") || "-"}
                  ...
                  <td>{sum(routeFills, "actual_qty")}</td>
                  <td>{lyd(sum(routeSales, "total_amount_lyd"))}</td>
                  <td>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <section className="surface-card p-4">
          ...
          {!sales.length ? (
            <EmptyState
              title="No manual sales"
              body="Manual route sales will appear here."
            />
          ) : (
            <DataTable
              headers={[
            ...
            >
              {sales.map((sale: any) => (
                <tr key={sale.id}>
                  <td>{time(sale.sale_time)}</td>
                  <td>
                    {sale.machine_id ? (
                      <Link
                        className="link-secondary"
                        href={`/machines/${sale.machine_id}`}
                      >
                        {formatMachineDisplayName(
                          machineById.get(sale.machine_id) ?? null,
                          { includeArea: true },
                        )}
                      </Link>
                    ) : (
                  ...
                  <td>{sale.quantity}</td>
                  <td>{lyd(Number(sale.total_amount_lyd ?? 0))}</td>
                  <td>
                    <StatusBadge
                      status={historyStatus(sale.status) || "recorded"}
                    />
                  </td>
                  <td>
                    <Link
                      className="link-secondary"
                      href={`/routes/${sale.route_id}`}
                    >
                      Route
                    </Link>
                  </td>
        ...
        <section className="surface-card p-4">
          ...
          {!fills.length ? (
            <EmptyState
              ...
              body="Completed fill activity will appear here."
            />
          ) : (
            <DataTable headers={["Time", "Machine", "Product", "Qty", "Route"]}>
              {fills.map((row: any) => (
                <tr key={row.id}>
                  ...
                  <td>
                    {row.machine_id ? (
                      <Link
                        className="link-secondary"
                        href={`/machines/${row.machine_id}`}
                      >
                        {formatMachineDisplayName(
                          machineById.get(row.machine_id) ?? null,
                          { includeArea: true },
                        )}
                      </Link>
                    ) : (
                  ...
                  <td>
                    {row.product?.name ?? row.missing_product_name ?? "Product"}
                  </td>
      ...
      <section className="surface-card p-4">
        <h2 className="text-lg font-semibold">
          Damaged, returned, and machine storage
        </h2>
        {!adjustments.length && !machineStorage.length ? (
          <EmptyState
            title="No product adjustments"
            body="Operator-assigned damaged, returned, and machine-storage records will appear here."
          />
        ) : (
          <DataTable
            headers={[
          ...
          >
            {[
              ...adjustments.map((row: any) => ({
                ...
                status: historyStatus(row.status) || "recorded",
                route_id: row.route_id,
                reason: row.reason ?? row.notes,
              })),
              ...machineStorage.map((row: any) => ({
                id: row.id,
                time: row.created_at,
                type: "machine_storage",
                machine_id: row.related_machine_id,
                product: row.product?.name,
                quantity: row.quantity,
                status: "recorded",
                route_id: row.related_route_id,
                reason: row.reason,
              })),
            ]
              .sort((a: any, b: any) =>

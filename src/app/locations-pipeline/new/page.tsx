...
export default async function NewLocationPipelineLeadPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return (
    <FormPageLayout>
      <PageHeader
        title="New CRM Lead / جهة جديدة"
        subtitle="Add an inbound inquiry or a place Snacky wants to approach, then assign the next action or field visit."
        breadcrumbs={[{ label: "Snacky CRM", href: "/locations-pipeline" }, { label: "New lead" }]}
        action={<SecondaryButton href="/locations-pipeline">Back to CRM</SecondaryButton>}
      />

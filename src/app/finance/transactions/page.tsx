...
function categoryLabel(row: any) {
  return row.category ?? row.final_bucket ?? row.transaction_type ?? row.bucket ?? String(row.transaction_kind ?? "transaction").replaceAll("_", " ");
}
...
function isProductPurchaseRow(row: any) {
  const category = String(row.category ?? row.final_bucket ?? row.transaction_type ?? "").trim().toLowerCase();
  return row.direction === "money_out"
...
function isActiveProductPurchaseRow(row: any) {
  return isProductPurchaseRow(row) && normalizedTransactionStatus(row) === "active";
}
...
async function financeLedgerDiagnostics({
  ...
  return {
    ...
    auth_error: authResult.error ? supabaseErrorDetails(authResult.error) : null,
...
function applyFinanceTransactionFilters({
  ...
  let nextQuery = query;
  if (statusFilter !== "all" && level !== "minimal") nextQuery = nextQuery.eq("transaction_status", statusFilter);
  if (params.review === "needs_review") nextQuery = nextQuery.eq("needs_review", true);
  if (params.review === "confirmed") nextQuery = nextQuery.eq("review_status", "confirmed");
  if (params.review === "reviewed") nextQuery = nextQuery.eq("review_status", "reviewed");
  ...
  if (params.date_from) nextQuery = nextQuery.gte("transaction_date", params.date_from);
  if (params.date_to) nextQuery = nextQuery.lte("transaction_date", params.date_to);
  if (search) {
    const pattern = supabaseLikePattern(search.replaceAll(",", " "));
    const searchableColumns = level === "minimal"
      ? ["transaction_kind", "transaction_type", "description", "final_bucket", "source_sheet"]
      : ["transaction_kind", "transaction_type", "description", "notes", "final_bucket", "payment_method", "source_sheet"];
    if (level === "full") searchableColumns.push("category", "counterparty_text", "payer_text", "paid_to_text");
    const clauses = searchableColumns.map((column) => `${column}.ilike.${pattern}`);
    if (matchingPurchaseIds.length) {
      clauses.push(`related_purchase_id.in.(${matchingPurchaseIds.join(",")})`);
      if (level === "full") {
        clauses.push(`linked_purchase_id.in.(${matchingPurchaseIds.join(",")})`);
        clauses.push(`and(source_type.eq.purchase,source_id.in.(${matchingPurchaseIds.join(",")}))`);
      }
    }
...
export default async function FinanceTransactionsPage({
  ...
  const matchingPurchaseIds = ((purchaseSearchResult.data ?? []) as any[]).map((purchase) => purchase.id).filter(Boolean);
  ...
  if (result.error && isColumnOrSchemaError(result.error)) {
    const firstDiagnostics = await financeLedgerDiagnostics({ supabase, profile, canEdit, selectedColumns, error: result.error });
    console.error("[finance] Failed to load transactions with full schema select", firstDiagnostics);
    ...
    if (stableResult.error && isColumnOrSchemaError(stableResult.error)) {
      console.error("[finance] Failed to load transactions with stable schema select", await financeLedgerDiagnostics({ supabase, profile, canEdit, selectedColumns, error: stableResult.error }));
      ...
      if (!minimalResult.error) {
        ledgerWarning = "Finance ledger loaded with legacy core fields only. Optional fields such as currency, account, notes, and linked source details are unavailable until the latest finance migrations are applied.";
      }
    }
  }

  if (result.error) {
    const diagnostics = await financeLedgerDiagnostics({ supabase, profile, canEdit, selectedColumns, error: result.error });
    ...
    return (
      <>
        <ErrorState
          title={permissionFailure ? "Finance permission required" : "Could not load transactions"}
          body={
            permissionFailure
              ? `Supabase denied SELECT on ${FINANCE_TRANSACTIONS_TABLE} for this user. Current role: ${profile.role ?? "unknown"}; roles: ${(profile.roles ?? []).join(", ") || "none"}. Admin or Finance access is required.`
              : `Supabase returned ${details.code ?? "an error"} while loading ${FINANCE_TRANSACTIONS_TABLE}: ${String(details.message ?? "Unknown database error")}. Selected columns: ${selectedColumns.join(", ")}.`
          }
          action={<SecondaryButton href="/finance/transactions">Retry</SecondaryButton>}
        />
      </>
    );
  }
  const baseRows = ((result.data ?? []) as any[]).map(normalizeFinanceTransactionRow);
  ...
  const [purchases, routes, machines, locations] = await Promise.all([
    fetchByIds(supabase, "purchase_orders", "id, receipt_number, order_date, supplier:suppliers(name)", baseRows.map(relatedPurchaseId).filter(Boolean)),
  ...
  const rows = loadLevel === "minimal" && statusFilter !== "all" ? baseRows.filter((row) => normalizedTransactionStatus(row) === statusFilter) : baseRows;
  ...
  const detailedCsvHref = csvDataHref(
    ...
    rows.map((row) => {
      ...
      return [
        ...
        row.currency ?? "LYD",
        row.transaction_effect === "transfer" ? `${accountLabel(row.source_account_id)} -> ${accountLabel(row.destination_account_id)}` : accountLabel(accountKeyFor(row)),
  ...
  const groupedCsvHref = csvDataHref(
    ...
    displayItems.map((item) => {
      ...
      const row = item.row;
      return [row.transaction_date, String(row.direction ?? "").replaceAll("_", " "), categoryLabel(row), Number(row.signed_amount ?? 0), row.currency ?? "LYD", 1, paymentLabel(row.payment_method), normalizedTransactionStatus(row), row.description ?? row.notes ?? ""];
    }),
  );
  ...
  return (
    <>
      ...
      {ledgerWarning ? <div className="mb-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-900">{ledgerWarning}</div> : null}

...
export default async function FinanceRentPage({ searchParams }: { searchParams: Promise<SearchParamsRecord> }) {
  return FilteredFinancePage({
    ...
    applyQuery: (query) => query.or(["transaction_kind", "transaction_type", "description", "notes", "final_bucket", "payment_method"].map((column) => `${column}.ilike.%rent%`).join(",")),

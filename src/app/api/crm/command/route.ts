...
export async function POST(request:Request){
 ...
 try{
  const payload=crmPayload(command.values);
  ...
  if(error){
   ...
   return NextResponse.json({ok:false,message:knownRollback?error.message:'Could not confirm the result. Retry the same saved request.',resetAllowed:knownRollback},{status:knownRollback?409:503});
  }
  ...
  return NextResponse.json({ok:true,id:data.id,kind:data.kind,href:crmHref(data.kind,data.id),message:'Saved in Snacky OS.'});
 }catch(error){

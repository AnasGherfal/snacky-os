...
export default async function OperatorRoutesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <details id="money-debts" className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <summary className="cursor-pointer list-none p-4 marker:hidden">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-base font-semibold text-slate-900">{t("Money & Debts")}</h2>
              <p className="mt-1 text-sm text-slate-500">{t("Buy from storage, record expenses, and review operator money without leaving the operator page.")}</p>
            </div>
            <span className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{t("Open tools")}</span>
          </div>
        </summary>

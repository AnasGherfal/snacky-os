...
export function leadFilters(params:LeadSearchParams):LeadFilters{
 ...
 for(const key of allowed){const value=params[key];if(typeof value==='string'&&value.trim())out[key]=value.trim().slice(0,4000);}
 if(out.offset&&!/^\d{1,6}$/.test(out.offset))delete out.offset;
 if(out.group&&out.status){
  if(out.status==='rejected'||out.status==='machine_placed'){out.group=out.status==='rejected'?'declined':'installed';delete out.focus;}
  else if(out.status==='accepted')out.group='agreed';
  else if(['declined','installed','agreed'].includes(out.group))out.group='active';
 }
 return out;
}
export function leadListHref(filters:LeadFilters,changes:LeadFilters={}):string{
 ...
 for(const key of allowed){if(key==='offset'&&!Object.hasOwn(changes,'offset'))continue;if(merged[key])query.set(key,merged[key]);}
 return '/locations-pipeline'+(query.size?'?'+query.toString():'');
}
export function leadPageRange(total:number,offset:number,count:number):string{return count>0?`${offset+1}–${offset+count} / ${total}`:`0 / ${total}`;}
export function leadDate(value:string|null|undefined,ar:boolean):string{
 ...
 const date=new Date(value+'T12:00:00Z');if(Number.isNaN(date.getTime())||date.toISOString().slice(0,10)!==value)return ar?'تاريخ غير صالح':'Invalid date';

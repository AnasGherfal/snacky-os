...
export type BalanceTransaction = {
  ...
  transfer_destination_amount?: number | string | null;
};
export const FINANCE_RECONCILIATION_CUTOFF_DATE = "2026-05-15";
export const RECONCILED_OPENING_BALANCES: FinanceBalances = { snacky_lyd: 9514, snacky_usd: 660, owner_lyd: -24360.5, owner_usd: -418 };
export const ZERO_FINANCE_BALANCES: FinanceBalances = { snacky_lyd: 0, snacky_usd: 0, owner_lyd: 0, owner_usd: 0 };
...
export function accountCurrency(accountId: string | null | undefined): FinanceCurrency { return String(accountId ?? "").endsWith("_usd") ? "USD" : "LYD"; }
export function accountLabel(accountId: string | null | undefined) { return FINANCE_ACCOUNTS.find((account) => account.id === accountId)?.label ?? "Snacky LYD"; }
...
function numeric(value: number | string | null | undefined) { const parsed = Number(value ?? 0); return Number.isFinite(parsed) ? parsed : 0; }
...
export function transactionImpacts(row: BalanceTransaction): Partial<FinanceBalances> {
  ...
  if (effect === "transfer") {
    ...
    let received = amount || Math.abs(signedAmount(row));
    if (accountCurrency(source) !== accountCurrency(destination)) {
      // The rate is display/audit metadata. Use the exact native received
      // amount, not a rounded rate and never an assumed 1:1 conversion.
      received = Number(row.transfer_destination_amount);
      if (!Number.isFinite(received) || received <= 0) throw new Error("Cross-currency transfer is missing its received amount. Review Finance before using this balance.");
    }
    return { [source]: roundMoney(-(amount || Math.abs(signedAmount(row)))), [destination]: roundMoney(received) };
  }
...
export function computeFinanceBalances(rows: BalanceTransaction[], openingBalances: Partial<FinanceBalances> = {}): FinanceBalances {
  ...
  for (const row of rows) {
    ...
    for (const account of FINANCE_ACCOUNTS) balances[account.id] = roundMoney(balances[account.id] + numeric(impacts[account.id]));
  }
  return balances;
}
export function computeFinanceBalancesFromCutoff({ rows, openingBalances = RECONCILED_OPENING_BALANCES, cutoffDate = FINANCE_RECONCILIATION_CUTOFF_DATE }: { rows: BalanceTransaction[]; openingBalances?: Partial<FinanceBalances>; cutoffDate?: string; }) {
  return computeFinanceBalances(rows.filter((row) => isFinanceLedgerTransaction(row, cutoffDate)), openingBalances);
}
export function financeTotalsByCurrency(balances: FinanceBalances) { return { LYD: roundMoney(balances.snacky_lyd + balances.owner_lyd), USD: roundMoney(balances.snacky_usd + balances.owner_usd) }; }

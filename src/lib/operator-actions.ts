...
function pickupPublicError(error: unknown) {
  ...
  if (code === "42883" || code === "PGRST202" || (text.includes("function") && (text.includes("snacky_confirm_route_pickup_batch_v3") || text.includes("validate_route_workflow_schema")))) {
...
function routeCompletionPublicError(error: unknown) {
  ...
  if (text.includes("route bag stock changed") || text.includes("ledger token")) return message;
...
export async function uploadRefillProofPhoto(formData: FormData) {
  ...
  const photoDigest = createHash("sha256")
...
export async function uploadInventoryAdjustmentPhoto(formData: FormData) {
  ...
  if (!clientSubmissionId || clientSubmissionId.length > 200) {
    throw new Error("A stable operation id is required before uploading an adjustment photo.");
  }
  ...
  const extension = safeFileSegment(originalName.split(".").pop() || "jpg", "jpg");
  const operationDigest = createHash("sha256").update(clientSubmissionId).digest("hex").slice(0, 24);
  const objectName = `${safeFileSegment(stopId, "stop")}-${adjustmentType}-${operationDigest}.${extension}`;
...
export async function confirmPickList(
  ...
  try {
    ...
    const { data: initialRouteStopItems, error: stopItemsError }: { data: any[] | null; error: any } = await supabase
      .from("route_stop_items")
      .select("id, route_stop_id, machine_id, product_id, planned_quantity")
      .eq("route_id", routeId);
    let routeStopItems = initialRouteStopItems;
    ...
    console.info("[operator:pick-list] Pickup confirmation attempt", {
      ...
      table_or_rpc: "rpc.snacky_confirm_route_pickup_batch_v3",
      ...logPickupConfirmationContext,
    ...
    ({ data: pickupRpcRows, error: pickupRpcError } = await supabase.rpc("snacky_confirm_route_pickup_batch_v3", {
      ...pickupRpcArgs,
...
export async function markStopInProgress(routeId: string, stopId: string) {
  try {
    ...
    const { supabase, profile, route } = await requireOperatorRouteAccess(routeId);
    if (!isActiveRouteStatus(route.status)) {
      throw new Error("This route is not active, so the stop cannot be started.");
    }
    ...
    if (String(stop.status ?? "") === ROUTE_STOP_IN_PROGRESS_STATUS) return { success: true };
    if (String(stop.status ?? "") !== ROUTE_STOP_PICKED_STATUS) {
      throw new Error(`This stop cannot be started from status ${String(stop.status ?? "unknown")}.`);
    }

    const { data: startedStop, error } = await supabase
      .from("route_stops")
    ...
    if (!startedStop) {
      const { data: currentStop, error: currentStopError } = await supabase
        ...
        .eq("id", stopId)
        .maybeSingle();
      if (currentStopError) throwActionError(currentStopError, "Could not verify whether this stop started.");
      if (String(currentStop?.status ?? "") === ROUTE_STOP_IN_PROGRESS_STATUS) return { success: true };
      throw new Error("This stop changed while it was being started. Refresh before continuing.");
    }
...
export async function skipStop(formData: FormData) {
  ...
  try {
    ...
    // Once the atomic inventory RPC has committed, skipping would hide a real
    // machine fill behind a misleading terminal status. The database trigger
    // installed with the completion recovery migration is authoritative; this
    // server-side preflight provides a useful message before attempting the
    // compare-and-set update.
    const completionWorkflowClient = getSupabaseAdminClient();
    if (!completionWorkflowClient) {
      throw new Error("The protected stop workflow is not configured.");
    }
    const { data: inventoryReceipt, error: inventoryReceiptError } = await completionWorkflowClient
      .from("route_stop_inventory_commits")
      .select("id")
      .eq("route_stop_id", stopId)
      .maybeSingle();
    if (inventoryReceiptError) {
      throwActionError(inventoryReceiptError, "Could not verify whether this stop already posted inventory.");
    }
    if (inventoryReceipt) {
      throw new Error("This stop already posted inventory and cannot be skipped. Retry completion or ask a supervisor to reconcile it.");
    }
    ...
    if (!after) {
      throw new Error("This stop changed while it was being skipped. Refresh the route and try again.");
    }
    ...
    await logActivity({
      ...
      idempotencyKey: `route-stop:${stopId}:skip`,
    });
    revalidateRouteWorkflow(routeId);
  } catch (error) {
...
function requireUuidValue(value: unknown, label: string) {
  const normalized = String(value ?? "").trim();
  if (!UUID_VALUE_PATTERN.test(normalized)) throw new Error(`${label} is invalid.`);
  return normalized;
}
...
export async function completeStop({
  ...
  const logCarriedBefore = new Map<string, number>();
  const logCarriedAfter = new Map<string, number>();
  ...
  try {
    ...
    logProfile = profile;
    const routeAccessProfile = await buildOperatorRouteAccessContext(supabase, profile);
    const requestedCompletionAt = new Date().toISOString();
    const clientStopSubmissionId = String(clientSubmissionId ?? "").trim() || `route-stop:${routeId}:${stopId}`;
    const requestedPhotoUrl = completionPhotoUrl?.trim() || null;
    const requestedPhotoPath = completionPhotoPath?.trim() || null;
    const photoUploadUnavailable = Boolean(completionPhotoUploadUnavailable)
      || Boolean(requestedPhotoPath?.startsWith("storage-unavailable/"));
    if (photoUploadUnavailable) {
      throw new Error("The final machine photo was not uploaded. Retry the photo upload before completing the stop.");
    }
    const hasNewCompletionPhoto = Boolean(requestedPhotoUrl || requestedPhotoPath);
    if (!routeId || !stopId || !machineId) throw new Error("Route, stop, and machine are required to complete a stop.");
    if (clientStopSubmissionId.length > 200) throw new Error("Stop submission id cannot exceed 200 characters.");
    ...
    // Route assignment has been verified above. Completion side effects are
    // intentionally hidden from the raw operator Data API. Require the
    // protected server client before inventory commits so a configuration
    // problem cannot leave a half-completed stop that can never recover.
    const completionWorkflowClient = getSupabaseAdminClient();
    if (!completionWorkflowClient) {
    ...
    const { data: stop, error: stopError } = await supabase
      .from("route_stops")
      .select("id, route_id, machine_id, status, completed_at, notes")
      .eq("id", stopId)
      .maybeSingle();
    if (stopError) throwActionError(stopError, "Could not load this stop.");
    if (!stop || stop.route_id !== routeId || stop.machine_id !== machineId) {
    ...
    const { data: existingProof, error: existingProofError } = await completionWorkflowClient
      .from("machine_refill_history")
      .select("id, refill_at, machine_photo_url, machine_photo_path, linked_issue_id, issue_notes")
    ...
    // Existing terminal/proof timestamps remain authoritative. For the first
    // attempt, the inventory receipt timestamp becomes the business event time
    // after the atomic commit returns below.
    const priorCompletedAt = stop.completed_at ?? existingProof?.refill_at ?? null;

    const existingPhotoUrl = String(existingProof?.machine_photo_url ?? "").trim() || null;
    const existingPhotoPath = String(existingProof?.machine_photo_path ?? "").trim() || null;
    const hasExistingCompletionPhoto = Boolean(
      existingPhotoUrl
      || (existingPhotoPath && !existingPhotoPath.startsWith("storage-unavailable/")),
    );
    ...
    const savedPhotoUrl = requestedPhotoUrl || existingPhotoUrl;
    const savedPhotoPath = requestedPhotoPath || (
      existingPhotoPath?.startsWith("storage-unavailable/") ? null : existingPhotoPath
    );
    ...
    const normalizedIssue = issue?.issueType?.trim() && issue.description?.trim()
      ? {
      ...
      : null;
    const hasIssueReport = Boolean(
      normalizedIssue || existingProof?.linked_issue_id,
    );
    ...
    const rpcFillLines = [
      ...normalizedFilledItems.map((item, index) => ({
        action_type: "assigned_fill",
        product_id: requireUuidValue(item.productId, `Product on assigned fill ${index + 1}`),
        actual_qty: requireNonnegativeInteger(item.quantity, `Filled quantity ${index + 1}`),
        assigned_quantity: requireNonnegativeInteger(item.assignedQty ?? 0, `Assigned quantity ${index + 1}`),
        refill_order_line_id: item.refillOrderLineId
          ? requireUuidValue(item.refillOrderLineId, `Refill line on assigned fill ${index + 1}`)
          : null,
        unavailable: Boolean(item.unavailable),
    ...
    const rpcMachineStorageLines = normalizedExtraItems.map((item, index) => ({
      product_id: requireUuidValue(item.productId, `Product in machine storage row ${index + 1}`),
      actual_qty: requireNonnegativeInteger(item.quantity, `Machine storage quantity ${index + 1}`),
      reason: item.reason || "other",
      notes: item.notes || null,
    }));
    ...
    const workflowPayloadHash = createHash("sha256")
      ...
      .digest("hex");
    const workflowSubmissionId = `route-stop-workflow:v1:${workflowPayloadHash}`;

    if (isTerminalRouteStatus(route.status)) {
      // A lost-response retry is read-only and succeeds only when the complete
      // incoming business payload hashes to the exact immutable receipt. A
      // stale or changed form must never be told that its edits were applied.
      const { data: terminalReceipt, error: terminalReceiptError } = await completionWorkflowClient
        .from("route_stop_inventory_commits")
        .select("latest_submission_id, workflow_completed_at")
        .eq("route_stop_id", stopId)
        .maybeSingle();
      if (terminalReceiptError) {
        throwActionError(terminalReceiptError, "Could not verify the completed stop receipt.");
      }
      if (
        stop.status === ROUTE_STOP_COMPLETED_STATUS
    ...
    const rpcRouteId = requireUuidValue(routeId, "Route id");
    const rpcStopId = requireUuidValue(stopId, "Stop id");
    const rpcMachineId = requireUuidValue(machineId, "Machine id");

    const { data: stopInventoryData, error: stopInventoryError } = await supabase.rpc(
      "snacky_commit_route_stop_inventory_v1",
      {
        p_route_id: rpcRouteId,
        p_route_stop_id: rpcStopId,
        p_machine_id: rpcMachineId,
        p_submission_id: workflowSubmissionId,
        p_fill_lines: rpcFillLines,
        p_machine_storage_lines: rpcMachineStorageLines,
      },
    );
    if (stopInventoryError) {
      throwActionError(stopInventoryError, "Could not save machine stop inventory.");
    }

    const rawStopInventoryResult: unknown = Array.isArray(stopInventoryData) ? stopInventoryData[0] : stopInventoryData;
    if (!rawStopInventoryResult || typeof rawStopInventoryResult !== "object") {
      throw new Error("Machine stop inventory did not return a commit result.");
    }
    const stopInventoryResult = rawStopInventoryResult as Record<string, unknown>;
    if (
      stopInventoryResult.inventory_committed !== true
      || String(stopInventoryResult.route_id ?? "") !== rpcRouteId
      || String(stopInventoryResult.route_stop_id ?? "") !== rpcStopId
      || String(stopInventoryResult.machine_id ?? "") !== rpcMachineId
      || String(stopInventoryResult.submission_id ?? "") !== workflowSubmissionId
    ) {
      throw new Error("Machine stop inventory returned an invalid commit result.");
    }
    const inventoryCommitReceiptId = requireUuidValue(
      stopInventoryResult.commit_receipt_id,
      "Stop inventory commit receipt",
    );
    const inventoryCommittedAt = String(stopInventoryResult.inventory_committed_at ?? "").trim();
    if (!inventoryCommittedAt || !Number.isFinite(Date.parse(inventoryCommittedAt))) {
      throw new Error("Machine stop inventory returned an invalid commit timestamp.");
    }
    // A downstream timeout must not move this refill into another day/month.
    // Exact retries reuse the receipt's original committed_at value.
    const completedAt = priorCompletedAt ?? inventoryCommittedAt;
    const inventoryMovementCount = requireNonnegativeInteger(
      stopInventoryResult.movement_count ?? 0,
      "Inventory movement count",
    );
    const inventoryNeedsReview = Boolean(stopInventoryResult.needs_review);
    const stopInventoryProducts: unknown[] = Array.isArray(stopInventoryResult.products)
      ? stopInventoryResult.products
      : [];
    stopInventoryProducts.forEach((productValue) => {
      if (!productValue || typeof productValue !== "object") return;
      const product = productValue as Record<string, unknown>;
      const productId = String(product.product_id ?? "");
      if (!productId) return;
      logCarriedBefore.set(productId, Number(product.operator_bag_before ?? 0));
    ...
    let linkedIssueId: string | null = existingProof?.linked_issue_id
      ...
      : null;
    if (normalizedIssue) {
      const issueId = linkedIssueId ?? stableUuid(`route-stop-issue:${stopId}`);
      const { data: createdIssue, error: issueError } = await completionWorkflowClient
        .from("issues")
        .upsert({
          id: issueId,
          machine_id: machineId,
          issue_type: normalizedIssue.issueType,
      ...
      if (createdIssue) {
        ...
        await logActivity({
          ...
          entityLabel: normalizedIssue.issueType,
          afterData: createdIssue,
          metadata: { route_id: routeId, machine_id: machineId, operator_id: route.operator_id },
          summary: `Reported ${normalizedIssue.priority} machine issue during route stop`,
          idempotencyKey: `route-stop:${stopId}:issue:${createdIssue.id}`,
        });
      }
    }
    ...
    const refillHistoryPayload = {
      ...
      issue_notes: normalizedIssue?.description || existingProof?.issue_notes || null,
      ...
      raw_record: {
        ...
        client_submission_id: clientStopSubmissionId,
        workflow_submission_id: workflowSubmissionId,
        workflow_payload_hash: workflowPayloadHash,
        movement_count: inventoryMovementCount,
        inventory_needs_review: inventoryNeedsReview,
        inventory_commit_receipt_id: inventoryCommitReceiptId,
      },
      updated_at: requestedCompletionAt,
    };
    const refillHistoryId = existingProof?.id ?? stableUuid(`route-stop-refill-history:${stopId}`);
    const refillHistoryResult = await completionWorkflowClient
      .from("machine_refill_history")
      .upsert({ id: refillHistoryId, ...refillHistoryPayload }, { onConflict: "id" })
      .select(refillHistorySelect)
      .single();
    if (refillHistoryResult.error) {
      throwActionError(refillHistoryResult.error, "Could not save the machine refill proof.");
    }

    const refillHistory = refillHistoryResult.data;

    // This is the final commit marker. Refill-order, route, and stop statuses
    // change together only after every required idempotent side effect above
    // has saved successfully. A failure before this point leaves the stop open
    // and safe to retry with the same submission.
    const { data: stopWorkflowData, error: stopWorkflowError } = await completionWorkflowClient.rpc(
      "snacky_finalize_route_stop_workflow_v1",
      {
        ...
        p_client_submission_id: workflowSubmissionId,
      },
    );
    if (stopWorkflowError) {
      throwActionError(stopWorkflowError, "Could not finalize this stop workflow.");
    }
    const rawStopWorkflowResult: unknown = Array.isArray(stopWorkflowData)
      ? stopWorkflowData[0]
      : stopWorkflowData;
    if (!rawStopWorkflowResult || typeof rawStopWorkflowResult !== "object") {
      throw new Error("Stop workflow finalization did not return a commit result.");
    }
    const stopWorkflowResult = rawStopWorkflowResult as Record<string, unknown>;
    if (
      String(stopWorkflowResult.route_id ?? "") !== rpcRouteId
      || String(stopWorkflowResult.route_stop_id ?? "") !== rpcStopId
      || String(stopWorkflowResult.machine_id ?? "") !== rpcMachineId
      || String(stopWorkflowResult.stop_status ?? "") !== ROUTE_STOP_COMPLETED_STATUS
      || String(stopWorkflowResult.submission_id ?? "") !== workflowSubmissionId
      || String(stopWorkflowResult.inventory_commit_receipt_id ?? "") !== inventoryCommitReceiptId
    ) {
      throw new Error("Stop workflow finalization returned an invalid commit result.");
    }
    const nextFillingStatus = String(stopWorkflowResult.route_status ?? ROUTE_IN_PROGRESS_STATUS);
    ...
    await logActivity({
      ...
      metadata: { route_id: routeId, machine_id: machineId, operator_id: route.operator_id },
      summary: inventoryNeedsReview
        ? `Completed route stop with ${inventoryMovementCount} inventory movement rows and a discrepancy for review`
        : `Completed route stop with ${inventoryMovementCount} inventory movement rows`,
      idempotencyKey: `route-stop:${stopId}:complete`,
    });
...
type FinalizeRouteInventoryCount = {
...
type FinalizeRouteInventoryResult = ActionResult<{
  routeId: string;
...
/**
 * Atomically reconciles the physical route-bag count, returns counted stock,
 * records any discrepancy, and completes or cancels the route.
 */
export async function finalizeRouteInventory({
  routeId,
  counts,
  clientSubmissionId,
  storageLocationId,
  ...
  routeId: string;
  counts: FinalizeRouteInventoryCount[];
  clientSubmissionId: string;
  storageLocationId?: string | null;
  expectedLedgerToken: string;
  action?: "complete" | "cancel";
  reason?: string | null;
}): Promise<FinalizeRouteInventoryResult> {
  const fallback = {
  ...
  const supabase = await getAuthenticatedSupabaseServerClient();
  if (!supabase) return actionFailure("Database is not available.", { ...fallback, code: "NO_SUPABASE" });

  try {
    const profile = await getCurrentProfile();
    if (!profile || profile.active_status !== "active") {
      throw new Error("You must be signed in with an active account to complete this route.");
    }

    const normalizedRouteId = requireUuidValue(routeId, "Route id");
    const normalizedStorageLocationId = storageLocationId
      ? requireUuidValue(storageLocationId, "Storage location id")
      : null;
    const normalizedSubmissionId = String(clientSubmissionId ?? "").trim();
    if (!normalizedSubmissionId) throw new Error("Route inventory submission id is required.");
    if (normalizedSubmissionId.length > 200) throw new Error("Route inventory submission id cannot exceed 200 characters.");
    const normalizedLedgerToken = String(expectedLedgerToken ?? "").trim().toLowerCase();
    if (!/^[0-9a-f]{32}$/.test(normalizedLedgerToken)) {
      throw new Error("A valid route bag ledger token is required. Refresh the physical count screen.");
    }
    if (action !== "complete" && action !== "cancel") throw new Error("Route inventory action is invalid.");
    const normalizedAction: "complete" | "cancel" = action;
    const normalizedReason = String(reason ?? "").trim() || null;
    if (normalizedAction === "cancel" && !normalizedReason) {
      throw new Error("Enter a reason before cancelling this route.");
    }
    if (!Array.isArray(counts)) throw new Error("Route inventory counts are required.");

    const seenCounts = new Set<string>();
    const normalizedCounts = counts.map((count, index) => {
      const bagOwnerId = requireUuidValue(count?.bagOwnerId, `Bag owner on count ${index + 1}`);
      const productId = requireUuidValue(count?.productId, `Product on count ${index + 1}`);
      const countKey = `${bagOwnerId}:${productId}`;
      if (seenCounts.has(countKey)) throw new Error(`Count ${index + 1} duplicates another bag and product row.`);
      seenCounts.add(countKey);

      return {
        bag_owner_id: bagOwnerId,
        product_id: productId,
        counted_quantity: requireNonnegativeInteger(count?.countedQuantity, `Physical count ${index + 1}`),
        discrepancy_reason: String(count?.discrepancyReason ?? "").trim() || null,
      };
    });

    const { data, error } = await supabase.rpc("snacky_finalize_route_inventory", {
      p_route_id: normalizedRouteId,
      p_action: normalizedAction,
      p_storage_location_id: normalizedStorageLocationId,
      p_counts: normalizedCounts,
      p_reason: normalizedReason,
      p_client_submission_id: normalizedSubmissionId,
      p_expected_ledger_token: normalizedLedgerToken,
    });
    if (error) throwActionError(error, "Could not reconcile this route inventory.");

    const rawRow: unknown = Array.isArray(data) ? data[0] : data;
    if (!rawRow || typeof rawRow !== "object") {
      throw new Error("Route inventory finalization did not return a reconciliation result.");
    }
    const row = rawRow as Record<string, unknown>;

    const result = {
      routeId: String(row.route_id ?? normalizedRouteId),
      ...
      reconciliationStatus: String(row.reconciliation_status ?? "balanced"),
      returnedQuantity: requireNonnegativeInteger(row.returned_quantity ?? 0, "Returned quantity"),
      discrepancyQuantity: requireNonnegativeInteger(row.discrepancy_quantity ?? 0, "Discrepancy quantity"),
    ...
    if (!result.alreadyFinalized) {
      await logActivity({
        profile,
        action: normalizedAction === "cancel" ? "cancel_route_inventory" : "finalize_route_inventory",
        entityType: "route",
        entityId: result.routeId,
        entityLabel: `Route ${result.routeId.slice(0, 8)}`,
        afterData: {
          reconciliation_id: result.reconciliationId,
          route_status: result.routeStatus,
          reconciliation_status: result.reconciliationStatus,
          returned_quantity: result.returnedQuantity,
          discrepancy_quantity: result.discrepancyQuantity,
        },
        metadata: {
          count_rows: normalizedCounts.length,
          storage_location_id: normalizedStorageLocationId,
        },
        summary: normalizedAction === "cancel"
          ? "Reconciled route inventory and cancelled route"
          : result.reconciliationStatus === "needs_review"
            ? "Completed route with an inventory discrepancy awaiting review"
            : "Reconciled route inventory and completed route",
      });
    }

    revalidateRouteWorkflow(result.routeId);
    return actionSuccess(result);
  } catch (error) {
    console.error("[operator:finalize-route-inventory] Failed", {
      route_id: routeId,
      count_rows: Array.isArray(counts) ? counts.length : null,
      code: errorField(error, "code"),
      error,
    });
    return actionFailure(routeCompletionPublicError(error), {
      ...fallback,
      code: String(errorField(error, "code") ?? "ROUTE_INVENTORY_FINALIZATION_FAILED"),
    });
  }
}

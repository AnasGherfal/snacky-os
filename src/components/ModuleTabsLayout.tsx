...
export function ModuleTabsLayout({ children, profile }: { children: ReactNode; profile: ModuleTabsProfile }) {
  ...
  const moduleParam = searchParams.get("module") ?? (profile.role === "finance" && isPurchasesPath(pathname) ? "finance" : null);
  ...
  const visibleTabs = useMemo(
    ...
    [profile.active_status, profile.id, profile.role, profile.team_member_id, tabGroup],
  );

...
export function InvestorMoneyForm({kind,entityId,userId,remaining,receipts=[]}:Props) {
 ...
 useEffect(()=>{
  ...
  if(raw) {
   try {
    ...
    if(!parsed || typeof parsed!=='object' || Array.isArray(parsed) || Object.values(parsed).some((v)=>typeof v!=='string') || parsed.entity_id!==entityId || parsed.kind!==kind || !uuid.test(parsed.client_submission_id??'')) throw new Error('Invalid request');
    setSaved(parsed);setDraft(parsed);
   } catch {setBlocked(true);setNotice(tr('A saved money request could not be read. Review investor history before recording another one.','تعذر قراءة طلب مالي محفوظ. راجع سجل المستثمر قبل تسجيل عملية جديدة.'));}
  }
  setReady(true);
 },[key,entityId,kind]);
 ...
 const submit=async()=>{
  ...
  let request=saved;
  if(!request) {
   // A second open view should reuse any request submitted from the first view.
   try {const raw=localStorage.getItem(key);if(raw){const parsed=JSON.parse(raw);if(parsed.kind!==kind||parsed.entity_id!==entityId||!uuid.test(parsed.client_submission_id??''))throw new Error('Invalid saved request');request=parsed;}} catch {setBlocked(true);setNotice(tr('Check the saved request and history before retrying.','راجع الطلب المحفوظ والسجل قبل إعادة المحاولة.'));return;}
  }
 ...
 return <form action={submit} className="space-y-3" dir={ar?'rtl':'ltr'}>
  ...
  <fieldset disabled={!ready||busy||Boolean(saved)||blocked} className="grid min-w-0 gap-3 border-0 p-0 sm:grid-cols-2">
   {kind==='contribution'?<>
    <label className="text-sm sm:col-span-2">{tr('Was this capital already entered in Finance?','هل تم تسجيل رأس المال في المالية؟')}
     <select required className="field-input mt-1" value={draft.funding_mode} onChange={(e)=>change('funding_mode',e.target.value)}>
      <option value="">{tr('Choose explicitly','اختر طريقة التسجيل')}</option><option value="link">{tr('Yes — link the existing money-in','نعم — ربط حركة القبض الموجودة')}</option><option value="new">{tr('No — record a new money-in','لا — تسجيل حركة قبض جديدة')}</option>
     </select>
    </label>
    {draft.funding_mode==='link'?<label className="text-sm sm:col-span-2">{tr('Existing Finance receipt','حركة القبض الموجودة')}
     <select required className="field-input mt-1" value={draft.existing_finance_id} onChange={(e)=>{const row=receipts.find((r)=>r.id===e.target.value);if(row)setDraft((current)=>({...current,existing_finance_id:row.id,amount:Number(row.amount).toFixed(2),date:row.transaction_date,currency:row.currency,account:row.account_id,rate:row.currency==='LYD'?'1':String(row.exchange_rate_usd_to_lyd??'')}));else change('existing_finance_id','');}}>
      <option value="">{tr('Select the investor’s actual capital receipt','اختر حركة استلام رأس المال الفعلية')}</option>{receipts.map((row)=><option key={row.id} value={row.id}>{row.transaction_date} · {row.amount} {row.currency} · {row.description??row.id}</option>)}
     </select>
    </label>:null}
   </>:null}
   ...
   {kind==='contribution'&&draft.currency==='USD'?<label className="text-sm">{tr('Agreed historical LYD value per USD','القيمة المعتمدة بالدينار لكل دولار وقت الاستلام')}<input type="number" required min="0.000001" step="0.000001" className="field-input mt-1" value={draft.rate} onChange={(e)=>change('rate',e.target.value)} /></label>:null}

...
export function CrmLeadTable({rows,ar,selection}:{rows:LeadRow[];ar:boolean;selection?:{items:FocusSelection[];locked:boolean;toggle:(row:LeadRow)=>void}}){
  ...
  return <>
    <div className={styles.desktop}>
      <div className={styles.tableRegion} role="region" aria-label={ar?'جدول الجهات والزيارات؛ يمكن تمريره أفقياً':'Leads and visits table; scroll horizontally when needed'} tabIndex={0}>
        <table className={styles.table}>
          ...
          <colgroup>{selection?<col style={{width:52}}/>:null}<col style={{width:'23%'}}/><col style={{width:'18%'}}/><col style={{width:'13%'}}/><col style={{width:'23%'}}/><col style={{width:'12%'}}/><col style={{width:'11%'}}/></colgroup>
          <thead><tr>{selection?<th scope="col"><span className={styles.srOnly}>{ar?'اختيار':'Select'}</span></th>:null}{headings.map(h=><th key={h} scope="col">{h}</th>)}</tr></thead>
          <tbody>{rows.map(row=><tr key={row.id} className={row.focused?styles.focusRow:row.overdue?styles.lateRow:undefined}>
            {selection?<td>{select(row)}</td>:null}<th scope="row"><LeadName row={row} ar={ar}/></th>
    ...
    <ol className={styles.mobile} aria-label={ar?'الجهات والزيارات':'Leads and visits'}>
      {rows.map(row=><li key={row.id} className={row.focused?styles.focusRow:row.overdue?styles.lateRow:undefined}>
        <div className={styles.mobileHeading}>{select(row)}<LeadName row={row} ar={ar}/><Stage row={row} ar={ar}/></div>
        ...
        <dl className={styles.mobileMeta}><div><dt>{ar?'الموعد':'Due'}</dt><dd><Due row={row} ar={ar}/></dd></div><div><dt>{ar?'الموظف المسؤول':'Assigned to'}</dt><dd><Owner row={row} ar={ar}/></dd></div></dl>

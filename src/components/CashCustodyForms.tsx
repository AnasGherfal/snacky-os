...
export function CashVarianceResolutionForm({ action, id, clientSubmissionId }: { action: FormAction; id: string; clientSubmissionId: string }) {
  return (
    <LocalDraftForm action={action} formType="cash-variance-resolution" draftKeyParts={[id]} className="mt-5 space-y-4">
      ...
      <button className="btn-primary w-full">Approve resolution and release for banking</button>
    </LocalDraftForm>
  );
}

export type BankableCashBatch = {
  id: string;
  bagId: string;
...
export function CashBankDepositForm({
  action,
  batches,
  clientSubmissionId,
}: {
  action: FormAction;
  batches: BankableCashBatch[];
  clientSubmissionId: string;
}) {
  const [selected, setSelected] = useState<string[]>([]);
  const selectedTotal = useMemo(
    () => Math.round(batches.filter((batch) => selected.includes(batch.id)).reduce((sum, batch) => sum + batch.countedAmount, 0) * 100) / 100,
    [batches, selected],
  );

  return (
    <LocalDraftForm action={action} formType="cash-bank-deposit" draftKeyParts={[clientSubmissionId]} className="space-y-5">
      <input type="hidden" name="client_submission_id" value={clientSubmissionId} />
      <div className="surface-card">
        <h2 className="text-lg font-semibold text-slate-900">Reconciled batches in this deposit</h2>
        <p className="mt-1 text-sm text-slate-500">Select every sealed batch included on the one bank receipt. The deposit must equal their combined total exactly.</p>
        <div className="mt-4 space-y-2">
          {batches.map((batch) => (
            <label key={batch.id} className="flex items-center justify-between gap-4 rounded-lg border border-slate-200 bg-white p-3 text-sm">
              <span className="flex min-w-0 items-center gap-3">
                <input
                  ...
                  onChange={(event) => setSelected((current) => event.target.checked ? [...current, batch.id] : current.filter((id) => id !== batch.id))}
                />
                <span><span className="font-semibold text-slate-900">{batch.bagId}</span><span className="block text-xs text-slate-500">Reconciled {batch.reconciledAt ? new Date(batch.reconciledAt).toLocaleString() : "—"}</span></span>
              </span>
              <span className="shrink-0 font-semibold text-slate-900">{batch.countedAmount.toFixed(2)} LYD</span>
            </label>
          ))}
        </div>
        <div className="mt-4 rounded-lg bg-slate-950 p-4 text-white">
          <div className="text-xs font-semibold uppercase tracking-wide text-slate-300">Selected batch total</div>
          <div className="mt-1 text-3xl font-semibold">{selectedTotal.toFixed(2)} LYD</div>
        </div>
      </div>

      <div className="surface-card grid gap-4 md:grid-cols-2">
        <FormField label="Deposit date and time" required>
          <input name="deposited_at" type="datetime-local" required defaultValue={libyaNow()} className="field-input" />
        </FormField>
        <FormField label="Bank deposit amount LYD" required hint="Must match the selected batch total.">
          <input name="amount_lyd" type="number" min="0.01" step="0.01" required value={selectedTotal || ""} readOnly className="field-input bg-slate-50" />
        </FormField>
        <FormField label="Bank reference" required>
          <input name="deposit_reference" required className="field-input" />
        </FormField>
        <FormField label="Destination account" required>
          <input name="destination_account" required defaultValue="Snacky bank account" className="field-input" />
        </FormField>
        <FormField label="Bank receipt" required>
          <input name="evidence_file" type="file" accept={evidenceAccept} required className="field-input" />
        </FormField>
        <FormField label="Deposit notes">
          <textarea name="notes" rows={3} className="field-input" />
        </FormField>
      </div>
      <button className="btn-primary" disabled={!selected.length}>Record deposit and close selected batches</button>
    </LocalDraftForm>
  );
}

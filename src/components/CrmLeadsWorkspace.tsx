...
export async function CrmLeadsWorkspace({searchParams={}}:{searchParams?:LeadSearchParams}){
  ...
  try{
    ...
    let result=leadFocusEnabled?await db.rpc('snacky_crm_lead_desk_v1',{p_filters:filters}):await db.rpc('snacky_crm_workspace_v1',{p_section:'lead',p_id:null,p_filters:filters});
  ...
  const quick=[['all',tr('All permitted leads','كل الجهات المسموح بها'),{scope:'all',assigned_to:'',window:'all',focus:''}],['mine',tr('Assigned to me','المسند إليّ'),{scope:'mine',assigned_to:'',window:'all',focus:''}],['overdue',tr('Overdue follow-ups','المتابعات المتأخرة'),{window:'overdue',focus:''}]] as const;
  const activeQuick=filters.focus==='active'?'focus':filters.window==='overdue'?'overdue':currentOwner===context.me?'mine':!currentOwner?'all':'';
  ...
  return <section className={styles.workspace} dir={ar?'rtl':'ltr'} id="crm-leads">
    ...
    {context.focus_ready?<nav className={styles.groupTabs} aria-label={tr('Lead outcomes','حالة التعاون')}>{[['all','All stages','كل المراحل'],['active','Open opportunities','فرص مفتوحة'],['agreed','Agreed — awaiting placement','متفق عليها — بانتظار التركيب'],['installed','Installed','تم التركيب'],['declined','Declined','رفضت التعاون']].map(([key,en,arabic])=><Link key={key} className={(filters.group??'all')===key?styles.quickActive:styles.quickLink} href={leadListHref(filters,{group:key,status:'',focus:'',window:'all'})} aria-current={(filters.group??'all')===key?'page':undefined}>{tr(en,arabic)}</Link>)}</nav>:null}

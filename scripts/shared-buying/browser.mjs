...
try{
 ...
 await check('owner shares the existing whole-box planning list; lost creation response recovers without duplicate',async()=>{
  ...
  await owner.getByRole('button',{name:'Save & assign list',exact:true}).click();await owner.getByRole('button',{name:'Retry saved request',exact:true}).waitFor();assert.equal(receipt.ok,true);listId=request.list_id;assert.equal(sql(`select count(*) from buying_private.lists where id='${listId}'`),'1');

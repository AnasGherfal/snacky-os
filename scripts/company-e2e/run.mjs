// Actual Auth, PostgREST, Storage, production Next.js and Chromium. Loopback only.
...
async function check(name,fn){try{await fn();results.push({name,status:'passed'});console.log('PASS '+name);}catch(error){results.push({name,status:'failed',message:String(error.message).slice(0,1800)});throw error;}finally{writeFileSync(`${out}/results.json`,JSON.stringify({results,errors},null,2));}}
...
try{
 ...
 await check('actual login and full Company shell',async()=>{
  for(const page of [owner,crm,operator,finance]){await page.goto(app+'/company');await page.locator('.company-hub').waitFor();assert.ok(await page.locator('header').count());assert.ok(await page.locator('main').count());}
 });
 let itemId,fileHref;
 ...
 await check('real file remains private until reviewed publication, then authorized preview/download work',async()=>{
 ...
 await check('duplicates and stale revisions cannot overwrite; publication preserves older versions',async()=>{
 ...
 await check('lost response after a real commit can be recovered through the actual editor',async()=>{
  const before=await rpc(accounts.owner.client,'snacky_company_workspace_v1',{p_id:itemId});
  await owner.goto(app+`/company/items/${itemId}?edit=1`);await owner.getByLabel('Instructions / responsibilities (plain text)',{exact:true}).fill('Confirmed once after response loss.');
  let savedRequest;
 ...
 await check('archive and restore enforce current access without deleting history',async()=>{
 ...
 await check('direct API, bad origin, oversized JSON and role gates are enforced',async()=>{
  const w=await rpc(accounts.owner.client,'snacky_company_workspace_v1',{p_id:itemId});
 ...
 await check('deactivating the linked staff member revokes Company access despite an older active profile',async()=>{
  const id=accounts.crm.member;sql(`update public.team_members set active=false,active_status='inactive' where id='${id}'`);
  assert.ok((await accounts.crm.client.rpc('snacky_company_notices_v1',{})).error);
  assert.equal((await contexts.crm.request.get(app+fileHref)).status(),404);
  sql(`update public.team_members set active=true,active_status='active' where id='${id}'`);
 });
 await check('reusable library record shows real authorized file rather than a placeholder',async()=>{
 ...
 await check('full-shell Arabic/English mobile and desktop layout and accessible Company controls',async()=>{
  const violations=[];
  for(const [label,page,path,width] of [['home-ar-390',crm,'/company',390],['guide-ar-390',crm,`/company/items/${itemId}`,390],['library-en-1440',owner,'/company/documents',1440],['editor-en-320',owner,`/company/items/${itemId}?edit=1`,320]]){
   ...
   const audit=await new AxeBuilder({page}).include('.company-hub').analyze();violations.push({label,violations:audit.violations});
  }
  writeFileSync(`${out}/accessibility.json`,JSON.stringify(violations,null,2));
  assert.equal(violations.flatMap(a=>a.violations).filter(v=>['serious','critical'].includes(v.impact)).length,0,'Company accessibility violations');
 });
 await check('real Company commands leave existing route, inventory and money ledgers unchanged',async()=>{assert.equal(ledger(),moneyBefore);});

...
try {
  ...
  await check('real CRM lead conversion and operator-to-relations issue handoff remain connected', async () => {
    ...
    const dispatch = async (action, note) => {
      const current = await read('operator', 'task', task.id);
      const taskData = current.record.data;
      const command = { request_id: randomUUID(), task_id: task.id, action, version: taskData.version, ...(note === undefined ? {} : { note }) };
      return rpc(accounts.operator.client, 'snacky_issue_dispatch_command_v1', { p_command: command });
    };
    await dispatch('accept');
    await dispatch('en_route');
    ...
    await dispatch('fix', 'QA field action complete');

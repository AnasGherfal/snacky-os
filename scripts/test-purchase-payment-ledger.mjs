...
test("purchase pages show ledger-derived status and fail closed when unavailable", () => {
  ...
  assert.match(listPage, /<PurchaseTablePayment/);
  assert.doesNotMatch(listPage, /#record-supplier-payment/i);
  assert.match(listPage, /supplierPaymentState\(purchase, summary\)/);
  ...
  assert.match(detailPage, /supplierPaymentState/);
  assert.match(detailPage, /unavailable/i);
  assert.doesNotMatch(detailPage, /markPurchasePaid/);
});

test("inventory accounting tolerates only four-decimal unit-cost precision drift", () => {
  assert.equal(
